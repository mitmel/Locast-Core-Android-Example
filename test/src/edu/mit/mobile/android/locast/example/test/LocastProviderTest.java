...
public class LocastProviderTest extends ProviderTestCase2<LocastProvider> {
	...
	/**
	 * Adds a user's information -- {@link Authorable#COL_AUTHOR} and
	 * {@link Authorable#COL_AUTHOR_URI} -- to the given content values item.
	 *
	 * @param cv
	 * @param uri
	 */
	...
	/**
	 * Creates a new cast owned by the given user with the given title.
	 *
	 * @param cr
	 * @param user
	 * @param title
	...
	/**
	 * Creates a new collection owned by the given user with the given title.
	 *
	 * @param cr
	 * @param user
	 * @param title
	 * @return
	 */
	private Uri createCollection(ContentResolver cr, String user, String title) {
		final ContentValues cv = JsonSyncableItem.newContentItem();

		cv.put(Collection.COL_TITLE, title);
		addUserCv(cv, user);

		final Uri collection = cr.insert(Collection.CONTENT_URI, cv);

		return collection;
	}

	/**
	 * Adds the title and author information to the given {@link ContentValues}.
	 *
	 * @param cv
	 * @param user
	 * @param title
	...
	/**
	 * Test cast creation and authorable.
	 *
	 * @throws JSONException
	 */
	...
	public void testCreateCollection() throws JSONException {
		...
		final MockContentResolver cr = getMockContentResolver();
		final String user = createUser("Tag T.");

		final ContentValues cv = JsonSyncableItem.newContentItem();

		final String collectionTitle = "my collection";

		final Uri collection = createCollection(cr, user, collectionTitle);

		assertNotNull(collection);

		final Cursor c = cr.query(collection, null, null, null, null);

		assertTrue(c.moveToFirst());

		assertEquals(collectionTitle, c.getString(c.getColumnIndex(Collection.COL_TITLE)));
	...
	/**
	 * Tests updating tags on a cast.
	 *
	 * @throws JSONException
	 */
	public void testTagsUpdate() throws JSONException {
		final MockContentResolver cr = getMockContentResolver();
		final String user = createUser("Tag T.");
		...
		updateCv.put(Tag.TAGS_SPECIAL_CV_KEY, "bar,baz");

		final int count = cr.update(cast1, updateCv, null, null);

		assertEquals(1, count);

		assertTagsMatch(cr, CAST_PROJECTION, Cast.CONTENT_URI, 1, new long[] { 1 },
				new String[] { cast1Title }, "bar");
		assertTagsMatch(cr, CAST_PROJECTION, Cast.CONTENT_URI, 1, new long[] { 1 },
				new String[] { cast1Title }, "baz");

		assertTagsMatch(cr, CAST_PROJECTION, Cast.CONTENT_URI, 0, null, null, "foo");
	}

	/**
	 * Tests tagging on casts that are in a collection.
	 *
	 * @throws JSONException
	 */
	public void testCollectionCastTags() throws JSONException {
		final MockContentResolver cr = getMockContentResolver();
		final String user = createUser("Tag T.");
		...
		final String castTitle = "my cast";

		final ContentValues castCv = JsonSyncableItem.newContentItem();
		addCastCv(castCv, user, castTitle);

		castCv.put(Tag.TAGS_SPECIAL_CV_KEY, "foo,bar");

		final Uri collectionCasts = Collection.CASTS.getUri(collection);

		final Uri cast = cr.insert(collectionCasts, castCv);

		assertNotNull(cast);

		assertTagsMatch(cr, CAST_PROJECTION, collectionCasts, 1, new long[] { 1 },
				new String[] { castTitle }, "foo");

		assertTagsMatch(cr, CAST_PROJECTION, collectionCasts, 1, new long[] { 1 },
				new String[] { castTitle }, "bar");

		assertTagsMatch(cr, CAST_PROJECTION, collectionCasts, 0, null, null, "baz");

		final ContentValues updatedTags = new ContentValues();

		updatedTags.put(Tag.TAGS_SPECIAL_CV_KEY, "bar,baz");

		cr.update(cast, updatedTags, null, null);

		assertTagsMatch(cr, CAST_PROJECTION, collectionCasts, 0, null, null, "foo");

		assertTagsMatch(cr, CAST_PROJECTION, collectionCasts, 1, new long[] { 1 },
				new String[] { castTitle }, "bar");

		assertTagsMatch(cr, CAST_PROJECTION, collectionCasts, 1, new long[] { 1 },
				new String[] { castTitle }, "baz");
